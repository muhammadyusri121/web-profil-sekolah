...
interface TemplatSlugHalamanProps {
    title: string;
    basePath: string;
    apiUrl: string;
}

export default async function TemplatSlugHalaman({ title, basePath, apiUrl }: TemplatSlugHalamanProps) {
    let post = null;
    let errorStatus = 0;
    ...
    try {
        ...
        console.warn(`Gagal mengambil data untuk ${title}`, err);
        errorStatus = 500;
    }
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-yellow-50">
                <div className="text-center">
                    <h1 className="text-3xl font-black text-yellow-800">Server Error ({errorStatus})</h1>
                    <p className="text-yellow-500 mt-2">Gagal mengambil data dari database.</p>
                    <Link href={basePath} className="text-yellow-600 mt-4 font-semibold hover:underline block">
                        ← Kembali ke {title}
                    </Link>
                </div>
            </div>
    ...
    if (!post) {
        return (
            <div className="min-h-screen flex flex-col items-center justify-center bg-yellow-50">
                ...
                <Link href={basePath} className="text-yellow-600 mt-4 font-semibold hover:underline">
                    ← Kembali ke {title}
                </Link>
            </div>
        );
        ...
        <div className="min-h-screen flex flex-col bg-white">
            ...
            <main className="grow pt-32 pb-20 px-5 md:px-8">
                <article className="max-w-3xl mx-auto">
                    ...
                    <div className="mb-8">
                        <BackButton label={`Kembali ke ${title}`} />
                    </div>
