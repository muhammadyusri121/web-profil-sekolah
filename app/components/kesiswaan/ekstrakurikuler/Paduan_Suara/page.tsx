...
export default async function PaduanSuaraPage() {
    const baseUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:3000";
    const res = await fetch(`${baseUrl}/api/ekskul?ekskul_name=Paduan_Suara`, {
        cache: 'no-store'
    });
                    ...
                    {posts && posts.length > 0 ? (
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-5">
                            {posts.map((post: any) => (
                                <Link
                                    key={post.id}
                                    href={`/ekskul/paduan-suara/${post.slug}`}
                                    className="group flex flex-col bg-white rounded-2xl overflow-hidden border border-yellow-100 shadow-[0_2px_10px_rgb(0,0,0,0.02)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] hover:-translate-y-1 transition-all duration-300"
                                >
