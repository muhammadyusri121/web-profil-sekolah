...
export default async function DetailPaduanSuaraPage({ params }: { params: Promise<{ slug: string }> }) {
    const { slug } = await params;
    const baseUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:3000";

    try {
        const res = await fetch(`${baseUrl}/api/ekskul?slug=${slug}`, { cache: 'no-store' });
        ...
        if (!post) {
            return (
                <div className="min-h-screen flex flex-col items-center justify-center bg-yellow-50">
                    ...
                    <Link href="/ekskul/paduan-suara" className="text-yellow-600 mt-4 font-semibold hover:underline">
                        ← Kembali
                    </Link>
                </div>
            );
