...
export default function KurikulumPage() {
  const [pageInfo, setPageInfo] = useState<{ title: string; content: string }>({
    title: "Kurikulum",
    content: "Pantau seluruh agenda penting, hari libur, dan jadwal kegiatan sekolah dalam satu tempat yang praktis. Dirancang untuk memudahkan siswa dan guru dalam merencanakan waktu."
  });

  useEffect(() => {
    const fetchPageInfo = async () => {
      try {
        const res = await fetch('/api/post?slug=kurikulum');
        if (res.ok) {
