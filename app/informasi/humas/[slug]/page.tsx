...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
    ...
    try {
        ...
        const summary = post.content ? post.content.replace(/<[^>]*>/g, '').substring(0, 160) : "Detail informasi SMAN 1 Ketapang";

        return {
            title: `${post.title} | Humas SMAN 1 Ketapang`,
            description: summary,
            openGraph: {
                title: post.title,
                description: summary,
        ...
        return { title: "Detail Humas | SMAN 1 Ketapang" };
    }
}
...
export default async function DetailHumasPage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    return <TemplatSlugHalaman title="Humas" basePath="/informasi/humas" apiUrl={apiUrl} />;
}
